...
<xsl:variable name="drupalBasePath">/nextpls</xsl:variable>
      ...
      <table cellspacing="0" cellpadding="0" border="1"><tr>
        ...
        <div class="myTable">        
          <xsl:apply-templates select="itemTooltip" mode="plain">
            <xsl:with-param name="rootItem" select="'1'"/>
			<xsl:with-param name="hideSource" select="$hideSource"/>
		  </xsl:apply-templates>		  
        </div>
        </td>
...
<xsl:template name="itemTooltipTemplate">
    ...
    <xsl:if test="$rootItem and not($hideSource)">
        <xsl:apply-templates select="itemSource"/>
    </xsl:if>
    
    <xsl:if test="$rootItem">
...
<xsl:template name="printGemSocket">
    ...
    <xsl:choose>
        ...
        <xsl:otherwise>
            <xsl:choose>
                <xsl:when test="$color = 'Meta'"><br/><span class="setItemGray"><img src="{concat($drupalBasePath,'sites/all/modules/nprt/modules/tooltip/px/Socket_Meta.png')}" class="socketImg"/><xsl:value-of select="$loc/strs/itemTooltip/str[@id='armory.item-tooltip.meta-socket']"/></span></xsl:when>
                <xsl:when test="$color = 'Red'"><br/><span class="setItemGray"><img src="{concat($drupalBasePath,'/sites/all/modules/nprt/modules/tooltip/px/Socket_Red.png')}" class="socketImg"/><xsl:value-of select="$loc/strs/itemTooltip/str[@id='armory.item-tooltip.red-socket']"/></span></xsl:when>
                <xsl:when test="$color = 'Yellow'"><br/><span class="setItemGray"><img src="{concat($drupalBasePath,'/sites/all/modules/nprt/modules/tooltip/px/Socket_Yellow.png')}" class="socketImg"/><xsl:value-of select="$loc/strs/itemTooltip/str[@id='armory.item-tooltip.yellow-socket']"/></span></xsl:when>
                <xsl:when test="$color = 'Blue'"><br/><span class="setItemGray"><img src="{concat($drupalBasePath,'/sites/all/modules/nprt/modules/tooltip/px/Socket_Blue.png')}" class="socketImg"/><xsl:value-of select="$loc/strs/itemTooltip/str[@id='armory.item-tooltip.blue-socket']"/></span></xsl:when>
            </xsl:choose>
        </xsl:otherwise>
    </xsl:choose>
</xsl:template>
    ...
    <xsl:for-each select="spell"><br/>
        <xsl:choose>
            <xsl:when test="itemTooltip">
            	...
            	<br/><xsl:if test="(charges &gt; 1) or (charges != maxCharges)"><xsl:value-of select="charges"/> <xsl:value-of select="$loc/strs/itemTooltip/str[@id='armory.item-tooltip.charges']"/><br/></xsl:if>
            	<br/><xsl:apply-templates select="itemTooltip" mode="plain">
            		<xsl:with-param name="rootItem" select="'1'"/>
            		<xsl:with-param name="hideSource" select="'1'"/>
            ...
            <xsl:otherwise>
                <span class="bonusGreen">                
                    ...
                    </xsl:call-template><xsl:value-of select="$loc/strs/general/str[@id='colon']"/> <xsl:value-of select="desc"/>
				</span>
                <xsl:if test="(charges &gt; 1) or (charges != maxCharges)"><br/>
                	<xsl:value-of select="charges"/> <xsl:value-of select="$loc/strs/itemTooltip/str[@id='armory.item-tooltip.charges']"/>
				</xsl:if>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:for-each>
